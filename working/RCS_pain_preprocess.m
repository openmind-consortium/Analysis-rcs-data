...
%
% Seems to be working on MATLAB 2019b (previously we thought had to use
% 2019a or earlier)
%
...
%% COLLECT The state changes for durations > 2 hours, and Plot the features below
...
for x = 1:numruns
    ...
    if ~isempty(adbs.statechanges{mindurSub(x)})
        ...
        plot(adbs.statechanges{mindurSub(x)}.time,adbs.statechanges{mindurSub(x)}.newstate);
