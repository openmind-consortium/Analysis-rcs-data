...
set(gcf, 'Units', 'Normalized', 'OuterPosition', [1/3, 0, 1/3, 2/3]);
...
subplot(411)
title('Benchtop sinusoidal s(n), 25 \muV_{rms}, 20Hz')
yyaxis left, hold on
plot(t,keych)
ylabel('voltage (mV)')
yyaxis right
...
while counter <= round(totalTimeWindows*(1/2)) % loop through time singal
    if stime+L <= length(t) % check at least one time window available before reach end signal
        ...
        stem(fftBinsHz(1:length(YFFT)),YFFT,'color','k','LineWidth',2); hold on
        plot(fftBinsHz(1:length(YFFT)),YFFT,'color','k','LineWidth',1);
        title('fft last time window'), xlabel('Frequency bins (Hz)')
        ylabel('|S(k)| (rcs units)')
        set(gca,'fontsize', fontSize)
